def dot(a, b):
    ...
    if isinstance(a, list) and isinstance(b, list) and isinstance(a[0], (int, float)) and isinstance(b[0], (int, float)):
        if len(a) != len(b):
            raise ValueError(f"Dimensons mismatch for vector dot product: {len(a)} != {len(b)}")
    ...
    raise ValueError("Unsupported types for dot product")

def transpose(m):
    if not isinstance(m, list) or not isinstance(m[0], list):
        raise ValueError("Can only transpose a 2D matrix")
